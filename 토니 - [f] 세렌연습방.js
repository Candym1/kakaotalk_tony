var scriptName = "토니-세렌연습방";

/**
 * (string) room
 * (string) sender
 * (boolean) isGroupChat
 * (void) replier.reply("[세렌] 연습 트라이방", message)
 * (boolean) replier.reply("[세렌] 연습 트라이방", room, message, hideErrorToast = false) // 전송 성공시 true, 실패시 false 반환
...
function plus_team(msg, wait_team, create){
  ...
  if(msg == 1 || msg == 2){
    ...
    "\n\n1️⃣(초대:)\n2️⃣\n3️⃣\n4️⃣\n5️⃣\n6️⃣\n7️⃣\n8️⃣(리프:)\n9️⃣(은월:)\n🔟(숍:)" +
    "\n\n🥕메모 : "
    save("/메이플m/"+"/세렌연습방/"+"/명단/", wait_team.trim()+".txt", nomar_team)
...
function team_numbering(difficult){
  ...
  var all_team_list = java.io.File(sdcard+"/메이플m/"+"/세렌연습방/"+"/명단/").listFiles().join("\n");
  all_team_list = all_team_list.replace(/\/storage\/emulated\/0\/메이플m\/세렌연습방\/명단\//g,"");
  ...
  if(room=="[세렌] 연습 트라이방"){
    ...
    for(var no_k = 0; no_k < out_keyword2.length; no_k++){
      ...
      if(msg.indexOf(real_no)!=-1){
        replier.reply("[세렌] 연습 트라이방", sender + " 님\n" 
                      + "금지단어 적발! 채팅을 제한합니다🐰\n\n"
                      + "🚨경고방 자동 보고 시스템 진행🚨\n곧 운영진이 멘션 할거에요ㅡㅅㅡ")
        return
      }
    ...
    if(msg == "!1차경고"){
      if(sender.indexOf("뜡부")!=-1){
        replier.reply("[세렌] 연습 트라이방", "🚨신고 접수되어 선제재 적용됩니다🚨\n\n" 
                      + "🚨경고내용 : 1차경고\n\n"
    ...
    if(msg.startsWith("닉네임등록 ")){
      ...
      var r_nick = read("/메이플m/","/세렌연습방/"+"/닉네임/"+"닉네임모음.txt")
      if(r_nick.indexOf("}]")!=-1){
        var rr_nick = JSON.parse(r_nick)
        rr_nick[0][sender] = nick
          
        var re_nick = JSON.stringify(rr_nick)
        save("/메이플m/", "/세렌연습방/"+"/닉네임/"+"닉네임모음".trim() + ".txt", re_nick);
        
        replier.reply("[세렌] 연습 트라이방", sender + " 님의 닉네임\n" + nick + " 으로 등록 되었슘다🐰")
        return
      }else{
        replier.reply("[세렌] 연습 트라이방", "뜡부를 멘션해주세요! 🐰")
        return
      }
    }
    ...
    //외출관리
    if(msg == "외출"){
      var out_mem = read("/메이플m/"+"/세렌연습방/", "외출.txt")
      if(out_mem.indexOf(sender)!=-1){
        replier.reply("[세렌] 연습 트라이방", "🐰이미 외출명단 등록 하셨습니다!")
        return
      }else{
        ...
        save("/메이플m/"+"/세렌연습방/","외출.txt", string_out)
        out_name = [] 
        replier.reply("[세렌] 연습 트라이방", "🐰외출자 명단 등록하셨습니다!"+
        "\n또 놀러와염 ㅇ ㅅㅇ")
        return
      } 
    }
    ...
    if(msg == "명단수리"){
      down_time = false
      replier.reply("[세렌] 연습 트라이방", "오키오키 ㅇ ㅅㅇ🐰")
      return
    }
    ...
    if(msg == "전체명단" || msg == "명단" || msg == "연습명단"
      ...
      if(down_time == false){
        down_time = true
        var list = java.io.File(sdcard+"/메이플m/"+"/세렌연습방/"+"/명단/").listFiles().join("");
        list = list.replace(/\/storage\/emulated\/0\/메이플m\/세렌연습방\/명단\//g,"");
        ...
        if(msg == "전체명단" || msg == "명단"){
          ...
          if(ten == "팡"){
            replier.reply("[세렌] 연습 트라이방", "아무런 명단이 없어용🐰")
            down_time = false
            return
          }else{
            for(var i = 0; i < ten.length; i++){
              var all = read("/메이플m/","/세렌연습방/"+"전체명단.txt")

              if(all.indexOf("---")!=-1){
                var one = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten[i].trim() + ".txt")
                var real_one = all + "\n\n---------------\n\n" + one
                save("/메이플m/", "/세렌연습방/"+"전체명단.txt", real_one);   
              }else{
                var one2 = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten[i].trim() + ".txt")
                save("/메이플m/", "/세렌연습방/"+"전체명단.txt", "\n---🥕🐰🥕---\n\n" + one2);
              }
            }
            var allteam = read("/메이플m/"+"/세렌연습방/","전체명단.txt")
            replier.reply("[세렌] 연습 트라이방", "🐰전체명단이에요!🐰\n닉등록 및 명단작성은\n닉네임(직업)템셋 포스 기재!🥕"+줄이기+"\n\n"+allteam)
            save("/메이플m/", "/세렌연습방/"+"전체명단.txt", ",");
          ...
          if(ten_parc == "팡"){
            ...
            for(var i = 0; i < ten_parc.length; i++){
              var all_parc = read("/메이플m/","/세렌연습방/"+"연습명단.txt")

              if(all_parc.indexOf("---")!=-1){
                var one_parc = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten_parc[i].trim() + ".txt")
                var real_one_parc = all_parc + "\n\n---------------\n\n" + one_parc
                save("/메이플m/", "/세렌연습방/"+"연습명단.txt", real_one_parc);   
              }else{
                var one2_parc = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten_parc[i].trim() + ".txt")
                save("/메이플m/", "/세렌연습방/"+"연습명단.txt", "\n---🥕🐰🥕---\n\n" + one2_parc);
              }
            }
            var allteam_parc = read("/메이플m/"+"/세렌연습방/","연습명단.txt")
            replier.reply("[세렌] 연습 트라이방", "🐰연습명단이에요!🐰\n닉등록 및 명단작성은\n닉네임(직업)템셋 포스 기재!🥕"+줄이기+"\n\n"+allteam_parc)
            save("/메이플m/", "/세렌연습방/"+"연습명단.txt", ",");
          ...
          if(ten_sk == "팡"){
            ...
            for(var i = 0; i < ten_sk.length; i++){
              var all_sk = read("/메이플m/","/세렌연습방/"+"숙련명단.txt")

              if(all_sk.indexOf("---")!=-1){
                var one_sk = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten_sk[i].trim() + ".txt")
                var real_one_sk = all_sk + "\n\n---------------\n\n" + one_sk
                save("/메이플m/", "/세렌연습방/"+"숙련명단.txt", real_one_sk);   
              }else{
                var one2_sk = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten_sk[i].trim() + ".txt")
                save("/메이플m/", "/세렌연습방/"+"숙련명단.txt", "\n---🥕🐰🥕---\n\n" + one2_sk);
              }
            }
            var allteam_sk = read("/메이플m/"+"/세렌연습방/","숙련명단.txt")
            replier.reply("[세렌] 연습 트라이방", "🐰숙련명단이에요!🐰\n닉등록 및 명단작성은\n닉네임(직업)템셋 포스 기재!🥕"+줄이기+"\n\n"+allteam_sk)
            save("/메이플m/", "/세렌연습방/"+"숙련명단.txt", ",");
          ...
          if(ten_try == "팡"){
            ...
            for(var i = 0; i < ten_try.length; i++){
              var all_try = read("/메이플m/","/세렌연습방/"+"트라이명단.txt")

              if(all_try.indexOf("---")!=-1){
                var one_try = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten_try[i].trim() + ".txt")
                var real_one_try = all_try + "\n\n---------------\n\n" + one_try
                save("/메이플m/", "/세렌연습방/"+"트라이명단.txt", real_one_try);   
              }else{
                var one2_try = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten_try[i].trim() + ".txt")
                save("/메이플m/", "/세렌연습방/"+"트라이명단.txt", "\n---🥕🐰🥕---\n\n" + one2_try);
              }
            }
            var allteam_try = read("/메이플m/"+"/세렌연습방/","트라이명단.txt")
            replier.reply("[세렌] 연습 트라이방", "🐰트라이명단이에요!🐰\n닉등록 및 명단작성은\n닉네임(직업)템셋 포스 기재!🥕"+줄이기+"\n\n"+allteam_try)
            save("/메이플m/", "/세렌연습방/"+"트라이명단.txt", ",");
          ...
          if(ten_cle == "팡"){
            ...
            for(var i = 0; i < ten_cle.length; i++){
              var all_cle = read("/메이플m/","/세렌연습방/"+"클리어명단.txt")

              if(all_cle.indexOf("---")!=-1){
                var one_cle = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten_cle[i].trim() + ".txt")
                var real_one_cle = all_cle + "\n\n---------------\n\n" + one_cle
                save("/메이플m/", "/세렌연습방/"+"클리어명단.txt", real_one_cle);   
              }else{
                var one2_cle = read("/메이플m/","/세렌연습방/"+"/명단/"+ ten_cle[i].trim() + ".txt")
                save("/메이플m/", "/세렌연습방/"+"클리어명단.txt", "\n---🥕🐰🥕---\n\n" + one2_cle);
              }
            }
            var allteam_cle = read("/메이플m/"+"/세렌연습방/","클리어명단.txt")
            replier.reply("[세렌] 연습 트라이방", "🐰클리어명단이에요!🐰\n닉등록 및 명단작성은\n닉네임(직업)템셋 포스 기재!🥕"+줄이기+"\n\n"+allteam_cle)
            save("/메이플m/", "/세렌연습방/"+"클리어명단.txt", ",");
        ...
        replier.reply("[세렌] 연습 트라이방", "전체명단 작동 대기시간 3초 입니다!\n"+
        "3초뒤에 다시 입력해주세요 🐰")
        return
      }
    }
    ...
    if(msg == "명단생성"){
      if(team_wait.indexOf("팀")!=-1){
        replier.reply("[세렌] 연습 트라이방", "팀 생성 딜레이가 있어요!\n10초만 기다렸다 다시 꼬!🐰")
        return  
      }else{
        replier.reply("[세렌] 연습 트라이방", "원하는 팀을 선택해주세요!🐰\n"
        +"🥕 10초 내로 1~4 숫자만 입력\n"
        +"1. 세렌 연습팟\n"
        +"2. 세렌 숙련팟\n"
        +"3. 세렌 트라이팟\n"
    ...
    if(msg == "1" || msg == "2" || msg == "3" || msg == "4"){
      if(team_wait.indexOf("팀")!=-1){
        if(msg == "1"){
          ...
          var open_team = read("/메이플m/"+"/세렌연습방/"+"/명단/", real_wait_team.trim()+".txt")
          replier.reply("[세렌] 연습 트라이방", open_team)
    ...
    if(msg.indexOf("팀 명단")!=-1){
      ...
      if(real_list[0].find(element => element == team_num)){
        ...
        var see_team = read("/메이플m/"+"/세렌연습방/"+"/명단/", team_num.trim() + ".txt")
        replier.reply("[세렌] 연습 트라이방", see_team)
        real_list = []
        return
      }else{
        replier.reply("[세렌] 연습 트라이방", "롸??? 뭔팀이요??? 🐰")
        real_list = []
        return
      }
    }else if(msg.indexOf("팀명단")!=-1){
      replier.reply("[세렌] 연습 트라이방", "○팀 명단 으로 치세염ㅇㅅㅇ🐰")
      return
    }
    ...
    if (msg.indexOf("팀 삭제")!=-1){
      ...
      if(real_list[0].find(element => element == del_team)){
        new java.io.File(sdcard+"/메이플m/"+"세렌연습방/"+"/명단/"+ del_team.trim() + ".txt").delete();
        replier.reply("[세렌] 연습 트라이방", del_team + "이 삭제됐어요 🐰");
        real_list = []
        return;
      }else{
        replier.reply("[세렌] 연습 트라이방", "그런팀은 없습니다만?? ㅇ ㅅㅇ🐰");
        real_list = []
        return;
      }
    }
    ...
    if((msg.indexOf("!")!=-1) && (msg.indexOf("팀시간 ")!=-1)){
      ...
      if(real_list[0].find(element => element == real_team_time_name)){
        if(time_content.indexOf("욜")!=-1 || time_content.indexOf("모출")!=-1){
          var list_team = read("/메이플m/"+"/세렌연습방/"+"/명단/", real_team_time_name.trim() + ".txt");
          if(list_team.indexOf("⭐️ [시간] ex)")!= -1){
            if((time_content.indexOf("고정")!=-1) || time_content.indexOf("매주")!=-1){
              ...
              save("/메이플m/"+"/세렌연습방/"+"/명단/", real_team_time_name.trim() + ".txt", change_time);
        
              replier.reply("[세렌] 연습 트라이방", change_time);
              real_list = []
              return;
            }else{
            ...
            if((time_content.indexOf("고정")!=-1) || time_content.indexOf("매주")!=-1){
              ...
              save("/메이플m/"+"/세렌연습방/"+"/명단/", real_team_time_name.trim() + ".txt", change_2);
          
              replier.reply("[세렌] 연습 트라이방", change_2);
              real_list = []
              return
            }else{
          ...
          replier.reply("[세렌] 연습 트라이방", "날짜,요일,시간을 적어주세요 ㅡㅅㅡ\n"+
          "모출은 모출만 적어도 돼요!\n\n"+
          "ex) 11.20 화욜 2100\n요일은 욜로 통일[월욜, 화욜, 수욜] 🐰")
          return
        }
      }else{
      replier.reply("[세렌] 연습 트라이방", "없는팀 입니다만? ㅇ ㅅㅇ🐰");
      real_list = []
      return
      }
    }
    ...
    if(msg.indexOf("팀메모 ")!=-1){
      ...
      if(memo_content.indexOf("🥕메모 :")!=-1){
        replier.reply("[세렌] 연습 트라이방", "메모내용에 빼야할게 있어요!\n🥕메모 : << 이건 빼야해요🐰")
        return
      }else{
        var all_team_list = java.io.File(sdcard+"/메이플m/"+"/세렌연습방/"+"/명단/").listFiles().join("\n");
        all_team_list = all_team_list.replace(/\/storage\/emulated\/0\/메이플m\/세렌연습방\/명단\//g,"");
        ...
        if(real_list[0].find(element => element == team_name)){
          var team_content = read("/메이플m/"+"/세렌연습방/"+"/명단/", team_name.trim() + ".txt");
          ...
          if((memo_content.indexOf("고정")!=-1) || memo_content.indexOf("매주")!=-1){
            ...
            if(change_memt2.indexOf("🍒D+")!=-1){
              ...
              save("/메이플m/"+"/세렌연습방/"+"/명단/", team_name.trim() + ".txt", memo_complite_t);
              
              replier.reply("[세렌] 연습 트라이방", memo_complite_t);
              real_list = []
              return
            }
          }
          
          save("/메이플m/"+"/세렌연습방/"+"/명단/", team_name.trim() + ".txt", memo_complite);
        
          replier.reply("[세렌] 연습 트라이방", memo_complite);
          real_list = []
          return
        }else{
          replier.reply("[세렌] 연습 트라이방", "없는팀인데요 ㅇ ㅅㅇ?🐰");
          real_list = []
          return
        }
      }
    ...
    if(msg.indexOf("팀 메모삭제")!=-1){
      ...
      if(real_list[0].find(element => element == team_name1)){
        var team_content1 = read("/메이플m/"+"/세렌연습방/"+"/명단/", team_name1.trim() + ".txt");
        var memo_carrot1 = team_content1.split("🥕메모 : ")[1]
        var memo_complite1 = team_content1.replace("🥕메모 : " + memo_carrot1, "🥕메모 : " + "")
        save("/메이플m/"+"/세렌연습방/"+"/명단/", team_name1.trim() + ".txt", memo_complite1);
        
        replier.reply("[세렌] 연습 트라이방", memo_complite1);
        real_list = []
        return
      }else{
        replier.reply("[세렌] 연습 트라이방", "없는팀인데요 ㅇ ㅅㅇ?🐰");
    ...
    if(msg.indexOf("팀팀명 ")!=-1){
      ...
      if(real_list[0].find(element => element == team_name2)){
        var team_content2 = read("/메이플m/"+"/세렌연습방/"+"/명단/", team_name2.trim() + ".txt");
        ...
        save("/메이플m/"+"/세렌연습방/"+"/명단/", team_change_name.trim()+".txt", team_change_complite)
        replier.reply("[세렌] 연습 트라이방", team_change_complite)
        new java.io.File(sdcard+"/메이플m/"+"세렌연습방/"+"/명단/"+ team_name2.trim() + ".txt").delete();
        real_list = []
        return
      }else {
        replier.reply("[세렌] 연습 트라이방", "없는팀인데요 ㅇ ㅅㅇ?🐰");
    ...
    if((msg.indexOf("팀")!=-1) && (msg.indexOf("번")!=-1)){
      ...
      var bye_mem_mtr2 = read("/메이플m/","/경고/"+"세렌추방"+".txt");
      ...
      if(msg.indexOf("취소")!=-1){
        ...
        if(real_list[0].find(element => element == team2)){
          var content_team = read("/메이플m/"+"/세렌연습방/"+"/명단/", team2.trim() + ".txt");
          num2 = Number(num2)
          var num3 = num2 + 1
          ...
          if(content_team.indexOf(i_num + "(")!=-1){
            ...
            if(num2 == 10){
              if(carrotover2 == ""){
                replier.reply("[세렌] 연습 트라이방", "이미 빈칸이에요 ㅇㅅㅇ🐰")
                real_list = []
                return
              }else{
                ...
                replier.reply("[세렌] 연습 트라이방", del_name)
                save("/메이플m/"+"/세렌연습방/"+"/명단/", team2.trim() + ".txt", del_name);
              ...
              if(overlap_push2 == ""){
                ...
                replier.reply("[세렌] 연습 트라이방", del_name_1)
                save("/메이플m/"+"/세렌연습방/"+"/명단/", team2.trim() + ".txt", del_name_1);
            ...
            if(overlap_push_v == ""){
              ...
              replier.reply("[세렌] 연습 트라이방", del_name_2)
              save("/메이플m/"+"/세렌연습방/"+"/명단/", team2.trim() + ".txt", del_name_2);
          ...
          replier.reply("[세렌] 연습 트라이방", "없는 팀인데여...?ㅇㅅ ㅇ🐰")
          real_list = []
          return
        }
      }else if(msg.indexOf("삭제")!=-1){
        replier.reply("[세렌] 연습 트라이방", "○팀○번 취소\n위 내용처럼 해야해요🐰")
        real_list = []
        return
      }else{ //명단작성
        var nick_name = msg.split("번")[1].trim()
        var list_nick = read("/메이플m/","/세렌연습방/"+"/닉네임/"+"닉네임모음.txt")
        ...
        for(var w = 0; w < bye_mem.length + 1; w++){
          if(nick_name.indexOf(bye_mem[w])!=-1){
            replier.reply("[세렌] 연습 트라이방", "🐰 " + nick_name + "\n위 인원은 강퇴 제재 대상입니다.\n"
            +"문의사항 있다면 문의방 방문해주세요.")
            return
          }else if(sender.indexOf(bye_mem[w])!=-1){
            replier.reply("[세렌] 연습 트라이방", "🐰 " + sender + "\n위 인원은 강퇴 제재 대상입니다.\n"
            +"문의사항 있다면 문의방 방문해주세요.")
            return
          }
        }
        ...
        save("/메이플m/"+"/세렌연습방/", "팀명".trim() + ".txt", team2);
        save("/메이플m/"+"/세렌연습방/", "팀번호".trim() + ".txt", number2);
        save("/메이플m/"+"/세렌연습방/", "팀닉넴".trim() + ".txt", nick_name);
        var all_team_list = java.io.File(sdcard+"/메이플m/"+"/세렌연습방/"+"/명단/").listFiles().join("\n");
        all_team_list = all_team_list.replace(/\/storage\/emulated\/0\/메이플m\/세렌연습방\/명단\//g,"");
        all_team_list = all_team_list.replace(/\.txt/g,"")
        var rr = all_team_list.split("\n")
        ...
        if(real_list[0].find(element => element == team2)){
          ...
          var team_content3 = read("/메이플m/"+"/세렌연습방/"+"/명단/", team2.trim() + ".txt");
          ...
          if(nick_name == ""){
            ...
            if(nick_name2 !== undefined){
              for(var w = 0; w < bye_mem.length + 1; w++){
                if(nick_name2.indexOf(bye_mem[w])!=-1){
                  replier.reply("[세렌] 연습 트라이방", "🐰 " + nick_name2 + "\n위 인원은 강퇴 제재 대상입니다.\n"
                  +"문의사항 있다면 문의방 방문해주세요.")
            ...
            save("/메이플m/"+"/세렌연습방/", "팀번호".trim() + ".txt", number2_non);
            save("/메이플m/"+"/세렌연습방/", "팀닉넴".trim() + ".txt", nick_name2);
            ...
            if(nick_name2 !== undefined){
              if(team_content3.indexOf(i_number_non + "(")!=-1){
                ...
                if(number2_non == 10){
                  if(carrotover2 == ""){
                    ...
                    replier.reply("[세렌] 연습 트라이방", push_nick)
                    save("/메이플m/"+"/세렌연습방/"+"/명단/", team2.trim()+".txt", push_nick)
                    real_list = []
                    return
                  }else{
                    var memo_name = carrotover.split(" ")[0]
                    var carrotover3 = carrotover.split(memo_name)[1].trim()
                    save("/메이플m/"+"/세렌연습방/", "변경".trim() + ".txt", carrotover3);
                    
                    //명단덮기
                    replier.reply("[세렌] 연습 트라이방", carrotover3 + " 님의 자리!\n진짜 바꿔요?\n바꾼다면 10초내로 '네' 입력🐰")
                  ...
                  if(overlap_push2 == ""){
                    ...
                    var memo_name_1 = overlap_push.split(" ")[0]
                    var overlap_push3 = overlap_push.split(memo_name_1)[1].trim()
                    save("/메이플m/"+"/세렌연습방/", "변경".trim() + ".txt", overlap_push3);
                    
                    //명단덮기
                    replier.reply("[세렌] 연습 트라이방", overlap_push3 + " 님의 자리!\n진짜 바꿔요?\n바꾼다면 10초내로 '네' 입력🐰")
                ...
                if(overlap_push_v == ""){
                  ...
                  replier.reply("[세렌] 연습 트라이방", push_nick2)
                  save("/메이플m/"+"/세렌연습방/"+"/명단/", team2.trim()+".txt", push_nick2)
                  real_list = []
                  return      
                }else{
                  save("/메이플m/"+"/세렌연습방/", "변경".trim() + ".txt", overlap_push_v);
                  
                  //명단덮기
                  replier.reply("[세렌] 연습 트라이방", overlap_push_v + " 님의 자리!\n진짜 바꿔요?\n바꾼다면 10초내로 '네' 입력🐰")
              ...
              replier.reply("[세렌] 연습 트라이방", "없는 닉네임입니다?!\n닉네임등록 후 이용하시지요?ㅇㅅㅇ🐰")
              real_list = []
              return
            }
          ...
          //닉등록 안한 사람
            ...
            if(nick_null == ""){
              replier.reply("[세렌] 연습 트라이방", "번 뒤에 띄어쓰기 하지 마세여!\n○팀○번 으로 다시 작성!🐰")
              return
            }
          ...
          replier.reply("[세렌] 연습 트라이방", "없는 팀인데요? ㅇ ㅅㅇ?🐰")
          real_list = []
          return
        }
      }
      ...
      if(wait_dic[sender] !== undefined){
        for(var g = 0; g < 50; g++){
          if(yes_time == false){
            ...
            var team_content4 = read("/메이플m/"+"/세렌연습방/"+"/명단/", team_nname.trim() + ".txt")
            ...
            save("/메이플m/"+"/세렌연습방/"+"/명단/", team_nname.trim() + ".txt", change_name2)
            replier.reply("[세렌] 연습 트라이방", change_name2)
    ...
    if((msg.indexOf("팀")!=-1) && (msg.indexOf(" 초기화")!=-1)){
      ...
      if(reset_chec_fin == "초기화"){
        ...
        if(real_list[0].find(element => element == del_team_name)){
          var load_team = read("/메이플m/"+"/세렌연습방/"+"/명단/", del_team_name.trim() + ".txt")
          ...
          if(boss_name2 == "세렌"){
            ...
            var load_team2 = read("/메이플m/"+"/세렌연습방/"+"/명단/", del_team_name.trim() + ".txt")
            var changes1 = load_team2.replace("⭐️ [시간] ex) !•팀시간 ••","⭐️ "+real_time)
            var changes2 = changes1.replace(normal_memo, "🥕메모 : " + memo_material)
          
            replier.reply("[세렌] 연습 트라이방", changes2)
            save("/메이플m/"+"/세렌연습방/"+"/명단/", del_team_name.trim() + ".txt", changes2)
            real_list = []
            return
          }
        }else{
      ...
      for(var a = 0; a < sender_m.length; a++){
        if(sender.indexOf(sender_m[a])!=-1){
          ...
          if(real_list[0].find(element => element == crea_team)){
            var crea_content = read("/메이플m/"+"/세렌연습방/"+"/명단/", crea_team.trim() + ".txt");
            ...
            save("/메이플m/"+"/세렌연습방/"+"/명단/", crea_team.trim()+".txt", crea_change_complite)
            replier.reply("[세렌] 연습 트라이방", crea_change_complite)
          ...
          replier.reply("[세렌] 연습 트라이방", "관리자 불러오세여 ㅇㅅ ㅇ🐰")
          return
        }
      }
    }
    ...
    if(msg.indexOf("!가르치기") ==0){
      for(var a = 0; a < sender_m.length; a++){
        if(sender.indexOf(sender_m[a])!=-1){
          ...
          replier.reply("[세렌] 연습 트라이방", "🐰 토니가 아래내용을 기억해요"+줄이기+"\n"+suy1 +"을(를) " + suy2 +"(으)로 배웠어요🐰")
          save("/메이플m/"+"/세렌연습방/"+"/생각집/",suy1.trim()+".txt", suy2)
          return
        }
      }
      replier.reply("[세렌] 연습 트라이방", "🐰 관리자 불러오세요 ㅇㅅㅇ")
      return
    }
    
    //가르치기 대답
    var talk = read("/메이플m/"+"/세렌연습방/"+"/생각집/",msg+".txt")
    if(talk !== null){
  
      replier.reply("[세렌] 연습 트라이방", talk+" 🐰")
      return
    }
    ...
    if(msg.indexOf("!생각지우기") ==0){
      for(var a = 0; a < sender_m.length; a++){
        if(sender.indexOf(sender_m[a])!=-1){
          replier.reply("[세렌] 연습 트라이방", msg.substr(7) + "의 생각내용 : " + read("/메이플m/"+"/세렌연습방/"+"/생각집/",msg.substr(7) + ".txt"));
          var talk2 = read("/메이플m/"+"/세렌연습방/"+"/생각집/",msg.substr(7)+".txt")
          if(talk2 == null){
    
            replier.reply("[세렌] 연습 트라이방", "토니가 지울 생각이 없어요🐰")
            return
          }else if(talk2 !== null){
            new java.io.File(sdcard+"/메이플m/"+"/세렌연습방/"+"/생각집/" + msg.substr(7) + ".txt").delete()
            replier.reply("[세렌] 연습 트라이방", "토니가 "+"''"+msg.substr(7)+"''" + " 생각을 지웠어요🐰")
            return
          }
        }
      }
    ...
    if(msg=='!생각모음'){
      var list = java.io.File(sdcard+"/메이플m/"+"/세렌연습방/"+"/생각집/").listFiles().join("\n");
      list = list.replace(/\/storage\/emulated\/0\/메이플m\/세렌연습방\/생각집\//g,"");
      list = list.replace(/\.txt/g,"")
      
      replier.reply("[세렌] 연습 트라이방", "🐰토니의 생각들이에요\n"+줄이기+"\n\n"+list)
      return
    }
    ...
    if(msg.startsWith("!위치 ")){
      ...
      var pl_list = java.io.File(sdcard+"/메이플m/"+"/세렌연습방/"+"/명단/").listFiles().join("");
      pl_list = pl_list.replace(/\/storage\/emulated\/0\/메이플m\/세렌연습방\/명단\//g,"");
      
      if(pl_list == ""){
        replier.reply("[세렌] 연습 트라이방", "아무런 명단이 없어요!ㅇ ㅅㅇ🐰")
        return
      }
      ...
      for(var i = 0; i < pl_ten.length; i++){
        var check_pl = read("/메이플m/","/세렌연습방/"+"/명단/"+ pl_ten[i].trim() + ".txt")
      ...
      if(your_pl.length == 0){
        replier.reply("[세렌] 연습 트라이방", "아무팀에도 없는데여 ㅇㅅㅇ?🐰")
        return
      }
      var pl_send = "➖" + your_pl.join("\n➖")
      
      
      replier.reply("[세렌] 연습 트라이방", "🍒 [" + pl_name + "] 의 팀이에요!!\n\n"+
      pl_send+"\n\n"+
      "이제 그만 까먹으시죠 ㅇ ㅅㅇ 🐰\n\n"+
    ...
    if(msg.startsWith("!파티찾기 ")){
      ...
      if(partytt.indexOf(pl_party)!=-1){
        ...
        if(party_pl.length == 0){
          replier.reply("[세렌] 연습 트라이방", pl_party + "은 아무도 안가나봐요\n아니면 풀방이거나?! ㅇ ㅅㅇ🐰")
          return
        }else{
          for(var i = 0; i < party_pl.length; i++){
            var w_all = read("/메이플m/","/세렌연습방/"+"파티찾기.txt")

            if(w_all.indexOf("---")!=-1){
              var w_one = read("/메이플m/","/세렌연습방/"+"/명단/"+ party_pl[i].trim() + ".txt")
              var w_real_one = w_all + "\n\n---------------\n\n" + w_one
              save("/메이플m/", "/세렌연습방/"+"파티찾기.txt", w_real_one);   
            }else{
              var w_one2 = read("/메이플m/","/세렌연습방/"+"/명단/"+ party_pl[i].trim() + ".txt")
              save("/메이플m/", "/세렌연습방/"+"파티찾기.txt", "\n---🥕🐰🥕---\n\n" + w_one2);
            }
          }
          var w_allteam = read("/메이플m/"+"/세렌연습방/","파티찾기.txt")
          replier.reply("[세렌] 연습 트라이방", "🐰 [" + pl_party + "] 빈자리 팀이에요!!🐰\n\n생성자는 요일 안적으면\n"+
          "파티찾기로 명단안나옵니당ㅇㅅㅇ!🥕"+
        ...
        replier.reply("[세렌] 연습 트라이방", "월화수목금토일 모출\n위 단어로만 검색해주세요! 🐰")
        return
      }
    }
    ...
    replier.reply("[세렌] 연습 트라이방", "🐰토니의 사용법이에요 보세요!\n"+
    ...
    return
      ...
      replier.reply("[세렌] 연습 트라이방", "명령어 오류!!!\n"
      +"도움말 입력하여 봇사용법 보세요!\n"+
      "해결 안되면 뜡부 멘션주세요🐰"+e)
      return
    }
